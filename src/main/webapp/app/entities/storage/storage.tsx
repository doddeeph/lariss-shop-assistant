  ...
  return (
    <div>
      <h2 id="storage-heading" data-cy="StorageHeading">
        <Translate contentKey="larissShopAssistantApp.storage.home.title">Storages</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="larissShopAssistantApp.storage.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/storage/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="larissShopAssistantApp.storage.home.createLabel">Create new Storage</Translate>
          </Link>
        </div>
      </h2>
        ...
        {storageList && storageList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="larissShopAssistantApp.storage.id">ID</Translate>{' '}
            ...
            <tbody>
              {storageList.map((storage, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{storage.attributeLabel}</td>
                  <td>{storage.attributeName}</td>
                  <td>{storage.optionLabel}</td>
                  <td>{storage.optionValue}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/storage/${storage.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/storage/${storage.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="larissShopAssistantApp.storage.home.notFound">No Storages found</Translate>
            </div>
          )
        )}
      </div>
