...
public interface ProductRepository extends ReactiveCrudRepository<Product, Long>, ProductRepositoryInternal {
    ...
    @Query(
        "SELECT entity.* FROM product entity JOIN rel_product__variant joinTable ON entity.id = joinTable.variant_id WHERE joinTable.variant_id = :id"
    )
    Flux<Product> findByVariant(Long id);
